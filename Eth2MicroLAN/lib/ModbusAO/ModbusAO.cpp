...
bool ModbusAO::Reg(word address, word value) {
    ...
    if (reg->type == REG_RAM){
      *(reg->pValue) = value;
      ...
      reg->callback(1, (word)reg->pValue, (byte*)&value, sizeof(value));
    }else{
      return false;
    }
    return true;
...
word ModbusAO::Hreg(word offset) {
    return Reg(offset + OFFSET_HREG);
}
...
void ModbusAO::writeMultipleRegisters(byte* frame,word startreg, word numoutputs, byte bytecount) {
    //Check value
...
void ModbusAO::writeMultipleCoils(byte* frame,word startreg, word numoutputs, byte bytecount) {
    //Check value
